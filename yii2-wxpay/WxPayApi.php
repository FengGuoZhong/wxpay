...
use yii\WxPay\WxPayConfig;
use yii\WxPay\Data\WxPayResults;
use yii\WxPay\Data\WxPayReport;
//use yii\WxPayResults;
/**
 * 
...
class WxPayApi
{
	...
	/**
	 * 企业付款向微信零钱
	 * @param WxWorkPayOrder $inputObj
	 * @param number $timeOut
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */

	public static function workpay($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsMch_appidSet()){
			$inputObj->SetMch_appid(WxPayConfig::APPID);//商户账号APPID
			$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
		}
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function addReceiver($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/pay/profitsharingaddreceiver";
		//检测必填参数
		if(!$inputObj->IsMch_idSet()) {
			throw new WxPayException("缺少添加分账方接口必填参数mch_id！");
		}else if(!$inputObj->IsAppidSet()){
			throw new WxPayException("缺少添加分账方接口必填参数appid！");
		}
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function multiProfitSharing($inputObj, $timeOut = 6)
	{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function finishProfitSharing($inputObj, $timeOut = 6)
	{
